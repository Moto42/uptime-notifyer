	...
	if(data.method == 'get'){
		...
		if(trimmedAssetName.length >0) {
			...
			helpers.getStaticAsset(trimmedAssetName,function(err,data){
				if(!err&&data){
					//determing the content type (default to plain text)
						...
						_data.read('users',phone,function(err,userData){
							if(!err && userData){
								_data.delete('users',phone,function(err){
									if(!err){
										//Delete all the checks assosiated with this user.
												...
												_data.delete('checks',checkID,function(err){
													...
													if(checksDeleted == checksToDelete)
														if(!err){
															...
															callback(500,{'Error': 'error encountered while deleting user-checks, all checks may not have been deleted'})
														}
												});
											})
										...
										_data.read('users',checkData.userPhone,function(err,userData){
											if(!err && userData){
												...
												if(checkPosition>-1){
													...
													callback (500,{'error':'Check not found in user\'s list of checks'})
												}
											} else {
