...
workers.gatherAllChecks = function() {
	...
	_data.list('checks',function(err,checks){
		...
		if(!err && checks && checks.length > 0){
			checks = checks.filter( (check)=>check.indexOf('.json') >-1 );
...
workers.validateCheckData = function(originalCheckData) {
	...
	// if all the checks pass pass the data along to the next step  in the process
		...
		console.log("timeoutSeconds", originalCheckData.timeoutSeconds);
	if(
		originalCheckData.id &&
		originalCheckData.userPhone &&
		originalCheckData.protocol &&
...
workers.log = function(originalCheckData,checkOutcome,state,alertWarranted,timeOfCheck) {
	...
	_logs.append(logFileName,logString,function(err){
		...
		if(!err){
			console.log('Logging to file succeeded: ',logFileName);
		} else {
			console.log('Logging to file failed: ',logFileName);
		}
	})
		...
		if(!err && logs && logs.length >0){
			logs.forEach(function(logName){
				...
				_logs.compress(logID,newFileId,function(err){
					// @TODO ERROR IS SOMEWHERE IN HERE
					if(!err){
						//Truncate the log
