...
e.on('debugmode', function(str){ cli.responders.debugmode(str); });
...
cli.responders.listChecks = function(str) {
	const arr = str.split('--');
	const upDown = typeof(arr[1]) == 'string' && ['up','down'].indexOf(arr[1])>-1 ? arr[1] : false;
	switch(upDown){
		case false : cli.responders.listChecksAll() ; break;
		case 'up'  : cli.responders.listChecksUp()  ; break;
		case 'down': cli.responders.listChecksDown(); break;
	}
}

cli.responders.listChecksAll = function(){
...
cli.responders.listChecksUp = function(){
	_data.list('checks',function(err,checkList){
		if(!err && checkList){
			if(checkList.length >0){
				checkList.forEach((checkId)=>{
...
cli.responders.listChecksDown = function(){
	_data.list('checks',function(err,checkList){
		if(!err && checkList){
			if(checkList.length >0){
				checkList.forEach((checkId)=>{
...
cli.responders.moreCheckInfo = function(str) {
	const arr = str.split('--');
	const checkId = typeof(arr[1]) == 'string' && arr[1].length > 0 ? arr[1] : false;
	_data.read('checks',checkId, (err,checkData)=>{
		if(!err && checkData){
			console.log(checkData);
		} else {
			console.log('Error pulling check data. ',err);
		}
	})
}
...
cli.responders.moreLogInfo = function(str) {
	const arr = str.split('--');
	const fileId = typeof(arr[1]) == 'string' && arr[1].length > 0 ? arr[1] : false;
	if(fileId){
		cli.verticalSpace();
		// decompress the log
...
cli.responders.debugmode = function(str){
	const arr = str.split('--');
	const mode = typeof(arr[1]) == 'string' && arr[1].length > 0 ? arr[1] : false;
	if(mode){
		NODE_DEBUG = mode;
	} else {console.log('error setting new NODE_DEBUG value')}
}
