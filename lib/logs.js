...
lib.list = function(includeCompressedLogs,callback) {
	fs.readdir(lib.baseDir,function(err,data){
		if(!err && data){
			...
			//should we filter out the .gz.b64 files? If so, do so
			data = includeCompressedLogs ? data : data.filter((file)=>file.indexOf('.log')>-1);
			
			let trimmedFileNames = data;
			trimmedFileNames = trimmedFileNames.map( (file)=>file.replace('.log','') );
			trimmedFileNames = trimmedFileNames.map( (file)=>file.replace('.gz.b64','') );
...
//Read a log file into a string variable
